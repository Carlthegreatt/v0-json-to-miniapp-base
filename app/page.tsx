...
const LOOT_TABLE = [
  ...
  },
  {
    ...
    tier: "Rare",
    reward: "20-35 Coins",
    coins: Math.floor(Math.random() * 16) + 20,
  },
  {
    chance: 0.05,
    tier: "Bomb",
    reward: "BOOM! All coins lost!",
    coins: 0,
    isBomb: true,
  },
  {
  ...
  },
  {
    chance: 0.004,
    tier: "Mythic",
    reward: "Ultra Rare NFT + 200 Coins",
    coins: 200,
    isNFT: true,
    rarity: "Mythic",
  },
...
export default function NFTuklasApp() {
  ...
  const [livesTimer, setLivesTimer] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
    ...
    for (const item of LOOT_TABLE) {
      ...
      if (random <= cumulativeChance) {
        ...
        if (item.isNFT && item.rarity) {
          const rarityNames =
            NFT_NAMES[item.rarity as keyof typeof NFT_NAMES] ||
            NFT_NAMES.Common;
          result.nftName =
            rarityNames[Math.floor(Math.random() * rarityNames.length)];
        }
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg">
            Prospect for coins and rare NFTs on Base
          </p>
          {totalProspects > 0 && (
          ...
          {nftCollection.length > 0 && (
            <div className="flex items-center justify-center space-x-2">
              ...
              <span className="text-sm text-chart-5 font-medium">
                NFTs Collected: {nftCollection.length}
              </span>
            </div>
          )}
        </motion.div>
            ...
            <Card className="bg-card/80 backdrop-blur border-border/50 shadow-lg">
              <CardContent className="p-6 text-center space-y-4">
                ...
                <p className="text-muted-foreground">
                  Connect to Base network to start prospecting
                </p>
                  ...
                  <CardContent className="p-4 text-center">
                    <div className="flex items-center justify-center space-x-2 mb-2">
                      <motion.div
                        ...
                        animate={
                          showCoinAnimation ? { scale: [1, 1.2, 1] } : {}
                        }
                      >
                ...
                <Card className="bg-gradient-to-r from-chart-2/20 to-primary/20 backdrop-blur border-chart-2/50 shadow-lg">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      ⚠️ Bombs will destroy all prospect run coins!
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
                  ...
                  <Card className="bg-card/80 backdrop-blur border-border/50 shadow-lg">
                    ...
                    <CardContent className="text-center space-y-3">
                      ...
                      >
                        {lastResult.reward}
                        {lastResult.nftName && (
                          <span className="block text-chart-5 font-bold mt-1">
                            "{lastResult.nftName}"
                          </span>
                        )}
                      </motion.p>
                      {lastResult.coins > 0 && !lastResult.isBomb && (
                        ...
                        >
                          ...
                          {lastResult.isNFT ? "(banked)" : "(to prospect run)"}
                          {multiplier > 1 && !lastResult.isNFT && (
                            <span className="block text-chart-2 text-sm">
                              ({Math.floor(lastResult.coins / multiplier)} base
                      ...
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          onClick={() =>
                            setShopQuantities((prev) => ({
                              ...prev,
                              lives1: Math.max(1, prev.lives1 - 1),
                            }))
                          }
                        >
                          -
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setShopQuantities((prev) => ({
                              ...prev,
                              lives1: Math.min(10, prev.lives1 + 1),
                            }))
                          }
                        >
                          +
                        ...
                        <Button
                          onClick={() => buyLives(shopQuantities.lives1)}
                          size="sm"
                        >
                          Buy
                        </Button>
                      ...
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          onClick={() =>
                            setShopQuantities((prev) => ({
                              ...prev,
                              lives5: Math.max(1, prev.lives5 - 1),
                            }))
                          }
                        >
                          -
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setShopQuantities((prev) => ({
                              ...prev,
                              lives5: Math.min(5, prev.lives5 + 1),
                            }))
                          }
                        >
                          +
                        ...
                        <Button
                          onClick={() => buyLives(5 * shopQuantities.lives5)}
                          size="sm"
                        >
                          Buy
                        </Button>
              ...
              <Card className="bg-card/60 backdrop-blur border-border/30">
                <CardContent className="p-4 text-center">
                  <p className="text-muted-foreground text-sm">
                    🪙 1000 Coins = 0.01 ETH (when live)
                  </p>
                  <p className="text-muted-foreground text-xs mt-1">
                    Built on Base • Powered by v0
                  </p>
                </CardContent>
              </Card>
            </motion.div>
